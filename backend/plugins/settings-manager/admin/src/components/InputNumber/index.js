...
import {includes, isEmpty, isObject, map, mapKeys, reject, union, uniqBy} from 'lodash';
...
class InputNumber extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  renderFormattedInput = (handleBlur, inputValue, placeholder, marginBottom) => (
    < FormattedMessage
  id = {`settings-manager.${placeholder}`
}
...
render()
{
  ...
  return (
    < div
  className = {`${this.props.styles.inputNumber} ${requiredClass} ${bootStrapClass} ${bootStrapClassDanger}`
}>
<
  label
  htmlFor = {this.props.name} > < FormattedMessage
