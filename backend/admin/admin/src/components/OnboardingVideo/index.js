...
class OnboardingVideo extends React.Component {
  ...
  <
    ...
    className = {styles.txtWrapper} >
      < p
    className = {styles.title} > {video.title} < /p>
      < p
    className = {styles.time} > {isNaN(video.duration
  )
      ? '\xA0' : `${Math.floor(video.duration / 60)}:${Math.floor(video.duration) % 60}`
  }<
    /p>
    < /div>
    ...
    className = {styles.videoModalHeader}
      ...
      < /ModalHeader>
      < ModalBody
    className = {styles.modalBodyHelper} >
      < div >
      < Player
    ref = {this.player}
...

    {
      ...
      < div
      className = {cn(styles.hiddenPlayerWrapper
    )
    }>
    <
      Player
